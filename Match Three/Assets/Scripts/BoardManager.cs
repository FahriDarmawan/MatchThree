...
public class BoardManager : MonoBehaviour
{
    ...
    private void CreateBoard(Vector2 tileSize)
    {
        ...
        startPositon = (Vector2)transform.position - (totalSize / 2) + offsetBoard;
        endPosition = startPositon + totalSize;

        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                TileController newTile = Instantiate(tilePrefab, new Vector2(startPositon.x + ((tileSize.x + offsetTile.x) * x), startPositon.y + ((tileSize.y + offsetTile.y) * y)), tilePrefab.transform.rotation, transform).GetComponent<TileController>();
                tiles[x, y] = newTile;
    ...
    {
        Vector2 tileSize = tilePrefab.GetComponent<SpriteRenderer>().size;
        return new Vector2(startPositon.x + ((tileSize.x + offsetTile.x) * index.x), startPositon.y + ((tileSize.y + offsetTile.y) * index.y));
    }

    public List<TileController> GetAllMatches()
    {
        List<TileController> matcingTiles = new List<TileController>();
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                ...
                foreach(TileController item in tileMatched)
                {
                    ...
                    if (!matcingTiles.Contains(item))
                    {
                        matcingTiles.Add(item);
                    }
                }
            }
        }
    ...
    private void ProcessMatches()
    {
        ...
        combo++;
        ScoreManager.Instance.IncrementCurrenttScore(matchingTiles.Count, combo);
